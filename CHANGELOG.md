...
- The theme wipe swapped the remainder in one jump, leaving a wedge of the old theme in the far corner until it snapped. The cause was the radius: it was measured from `innerWidth`/`innerHeight`, which do not reliably describe the box the browser animates, so it could come up short. It is now a fixed `150vmax` — the farthest a corner can sit from a point inside the viewport is the diagonal, at most ~142vmax, so the circle always clears the whole snapshot. Only the centre still comes from JavaScript, clamped into the viewport.
