...
- The theme wipe swapped the remainder in one jump, leaving a wedge of the old theme in the far corner until it snapped. The radius was being sized against the viewport, but the box the browser clips is not the viewport — on a 2953px document in an 840px viewport it is the whole document, so the circle was covering roughly a third of what it needed to. It is now `circle(150%)`: a percentage on `circle()` resolves against its own reference box, so the radius tracks whatever box is actually clipped, with nothing measured in JavaScript. Covering a box from any point inside it needs at most its diagonal, 141.4%, so 150% always clears it. Only the centre still comes from JavaScript.
