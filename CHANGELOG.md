...
- The theme wipe swapped the remainder in one jump, leaving a wedge of the old theme in the far corner until it snapped. Fixed by inverting which snapshot is animated: the finished theme now sits underneath, complete and unclipped, and it is the *old* snapshot on top that gets masked away from the wordmark outwards. Whenever the transition ends — on time, early, or interrupted — what is already on screen is the finished state, so there is nothing left to snap. Clipping the new snapshot on top, as it did before, meant any radius that came up short left that wedge behind. The radius is also no longer measured in JavaScript (`innerWidth`/`innerHeight` do not reliably describe the box the browser animates); it is a fixed `150vmax`, which always clears the viewport diagonal.
