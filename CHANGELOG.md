...
- `BreadcrumbList` structured data on the client, project, and service detail pages, via a small `<JsonLd>` server component. Google's sitelinks guidance asks for "a logical site structure"; this states that hierarchy in machine-readable form instead of leaving it to be inferred from the link graph, and is what puts a breadcrumb line under a result in place of a raw URL.
...
- `npm run check:fit` — a text-fit check across 22 viewport widths in both languages. Everything here is Space Mono, and a monospace face makes rendered width arithmetic, so overflow is checkable without a browser. The model mirrors the components' real classes and asserts they are still present, so changing a component fails the check until the model is updated to match.
...
- The Organization `logo` points at `/apple-icon` (180x180) as an `ImageObject` rather than the 1200x630 open-graph banner, which is a social card rather than a logo and fails Google's logo guidance.
- The sitemap drops `changefreq` and `priority` — Google ignores both — and `lastmod` is a content-update constant rather than build time. A `lastmod` that reads "today" on every deploy is one search engines learn to discount.
...
- The stat grid broke its own text at 640px. `sm:` split it from two columns into four *and* stepped the values from 30px to 36px at the same breakpoint, so the cards dropped to 84px while the text grew — "2023" wrapped to "202"/"3", "Prishtinë" to "Pris"/"htin"/"ë". It holds two columns until `lg` now, and the city value no longer takes a second step up at `lg`, where four columns halve the card again. "Prishtinë" had been overflowing continuously from 640px to 1152px.
- The Albanian header row overflowed between 768px and 820px, where English fit — its nav labels run about 30% longer. The nav appears at `lg` rather than `md` now, with the drawer covering tablets.
...
- The theme wipe swapped the remainder in one jump, leaving a wedge of the old theme in the far corner until it snapped. The box the browser clips is the whole document, not the viewport — on a 2953px page in an 840px window it is all 2953px — so a radius sized against the viewport covered roughly a third of what it needed to. The radius is now the diagonal of that box, `hypot(scrollWidth, scrollHeight)`, set per switch: the diagonal is the furthest apart any two points in the box can be, so a circle that size covers it from wherever the centre sits. Both keyframes are lengths — mixing a length with a percentage is what makes the shape step instead of interpolate.
- The locale redirect sets `Vary: Accept-Language, Cookie`. Where `/` lands depends on the request, but nothing told caches that — nginx or a CDN in front of the app could store one visitor's `/en` redirect and serve it to everyone after them.
